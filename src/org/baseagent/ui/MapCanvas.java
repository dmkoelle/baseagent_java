...
public class MapCanvas extends Canvas {
    ...
    // fractional zoom relative to slippyZoom (1.0 = no fractional zoom). Range typically [0.5,2)
    ...
    public MapCanvas(Map map, int tileWidth, int tileHeight, int tileXSpacing, int tileYSpacing) {
        ...
        this.setOnScroll((ScrollEvent e) -> {
            ...
            if (e.isControlDown()) {
                ...
                while (zoomScale >= 2.0) { slippyZoom++; zoomScale /= 2.0; }
                while (zoomScale < 0.5)  { slippyZoom--; zoomScale *= 2.0; }

                // compute geographic point under cursor using previous scale and previous integer zoom
    ...
    public void setSimulation(org.baseagent.sim.Simulation sim) {
        this.gcc = new GridCanvasContext(sim, this.dummyGrid, null, this.gcc.getCellWidth(), this.gcc.getCellHeight(), this.gcc.getCellXSpacing(), this.gcc.getCellYSpacing());
    }
    ...
    public void update() {
        ...
        gc.save();
        // No global transform; tileRenderer and agents use viewOffsetX/viewOffsetY to compute screen positions
    ...
    public int getSlippyZoom() { return this.slippyZoom; }
