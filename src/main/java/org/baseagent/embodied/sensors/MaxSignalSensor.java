...
public class MaxSignalSensor extends EmbodiedSensor {
    ...
    // Search radius (cells) used when querying nearby beacons from Simulation
    // spatial index
    ...
    public void sense(Agent xagent) {
        ...
        List<Beacon> beacons = agent.getSimulation().getBeaconsNear(getLayerName(), agent.getCellX(), agent.getCellY(),
                this.searchRadius);
    ...
    public void setSearchRadius(int r) {
        this.searchRadius = Math.max(0, r);
    }
}