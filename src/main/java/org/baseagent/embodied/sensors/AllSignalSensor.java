 ...
 *
 * Example sensors: - Light sensor - Sound sensor - Pheromone sensor - Vision
 * sensor
 * 
 * You can really sense a bunch of things: - A signal in the environment -
 * Something physical about the environment, like the surface you're standing
 * on, or whether you're near another agent - Internal states
 * 
 * @author David Koelle
 */
public class AllSignalSensor extends EmbodiedSensor {
    ...
    public void sense(Agent agent) {
        ...
        List<Beacon> beacons = gridAgent.getSimulation().getBeacons().stream()
                .filter(beacon -> beacon.getGridLayer().getLayerName().equals(this.getGridLayer().getLayerName()))
