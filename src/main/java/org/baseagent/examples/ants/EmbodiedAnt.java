    ...
    public EmbodiedAnt(int nestX, int nestY) {
        ...
        setDrawable(new GridDrawable() {
            ...
            public void draw(GridCanvasContext gcc) {
                ...
                // antennae from head
                gc.strokeLine(midX + frontOffset + segW * 0.2, midY - segH * 0.3, midX + frontOffset + segW * 0.8,
                        midY - segH);
                gc.strokeLine(midX + frontOffset + segW * 0.2, midY + segH * 0.05, midX + frontOffset + segW * 0.8,
                        midY + segH * 0.4);
        ...
        // place a force effector (visualized in body) but movement is driven by
        // behavior forces
        ...
        addBehavior(new Behavior() {
            ...
            public void executeBehavior(Agent agent) {
                ...
                if (r == null)
                    r = 0.0;
                System.out.println("Ant#" + agent.getSimpleID() + " sensors L=" + String.format("%.2f", l) + " R="
                        + String.format("%.2f", r) + " pos=" + ((GridAgent) agent).getCellX() + ","
                        + ((GridAgent) agent).getCellY());
            }
        });

        // Main behavior: pickup/search/carry/return/wander using addForce for smooth
        // movement
        addBehavior(new Behavior() {
            @Override
            public void executeBehavior(Agent agent) {
                ...
                if (foundX >= 0) {
                    double dirToFood = org.baseagent.util.BaseAgentMath.direction(ant,
                            new org.baseagent.grid.GridPosition(foundX, foundY));
                    ant.addForce(new org.baseagent.util.Vector2D(0.05, dirToFood));
                    return;
                }

                // If pheromone present, bias toward it using left/right sensors
                double leftVal = left.getIntensityPort().getOutputValue() == null ? 0.0
                        : left.getIntensityPort().getOutputValue();
                double rightVal = right.getIntensityPort().getOutputValue() == null ? 0.0
                        : right.getIntensityPort().getOutputValue();
