...
public class Scheduler implements SimulationListener {
    ...
    public void runScheduledEvents(Simulation simulation) {
        List<ScheduledEvent> applicableEvents = scheduledEvents.stream().filter(event -> event.isApplicable(simulation))
                .collect(Collectors.toList());
        List<ScheduledEvent> applicableEventsInRunOrder = this.policy.organizeScheduledEvents(simulation,
                applicableEvents);
