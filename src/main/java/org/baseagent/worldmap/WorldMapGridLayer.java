...
/**
 * WorldMap-specific GridLayer backed by the Grid/GridLayer infrastructure. Adds
 * geographic bounds and a generic classification layer and remote lookup so
 * callers can obtain tokens (arbitrary strings) describing what's under the map
 * tiles. The class does not assume any particular token vocabulary.
 */
public class WorldMapGridLayer extends GridLayer<Object> {
    ...
    // Adapter Grid that proxies size queries to the WorldMap so GridLayerStep sizes
    // correctly
    private static class WorldMapGridAdapter extends Grid {
        private final WorldMap parentMap;

        WorldMapGridAdapter(WorldMap parentMap) {
    ...
    public GridLayer<?> getBackingGridLayer() {
        return this.backingGridLayer;
    }
    ...
    public int[] latLonToCell(double lat, double lon) {
        if (!isGridLayer())
            throw new IllegalStateException("Layer is not configured as a grid.");
    ...
    /**
     * Convert a classification-grid cell (col,row) to the geographic lat/lon at the
     * center of that cell. Returns {lat, lon}.
     */
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filename))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (tokens.length != expectedCols) {
                    throw new IOException("Inconsistent column count in classification file: expected " + expectedCols
                            + " but saw " + tokens.length);
                }
                rowsList.add(tokens);
            }
            if (rowsList.isEmpty())
                throw new IOException("Empty classification file: " + filename);
    ...
    public void setClassificationData(String[][] data) {
        if (data == null || data.length == 0 || data[0].length == 0)
            throw new IllegalArgumentException("Invalid classification data");
        this.rows = data.length;
        this.cols = data[0].length;
        this.classificationData = new String[this.rows][this.cols];
        for (int r = 0; r < this.rows; r++) {
            if (data[r].length != this.cols)
                throw new IllegalArgumentException("Inconsistent column counts in classification data");
    ...
    // Default: return first whitespace/comma-separated token from response,
    // trimmed.
    ...
    public void setClassificationServiceEndpointTemplate(String template) {
        this.classificationServiceEndpointTemplate = template;
    }

    public void setClassificationServiceZoom(int z) {
        this.classificationServiceZoom = z;
    }

    public void setClassificationServiceTimeoutMs(int ms) {
        this.classificationServiceTimeoutMs = ms;
    }

    public void setClassificationInterpreter(Function<String, String> interpreter) {
        if (interpreter != null)
            this.classificationInterpreter = interpreter;
    }

    public void clearExternalClassificationCache() {
        externalClassificationCache.clear();
    }

    private String buildClassificationServiceUrl(double lat, double lon, int zoom) {
        if (classificationServiceEndpointTemplate == null)
            return null;
    ...
    public String queryClassificationService(double lat, double lon, int zoom) {
        ...
        if (externalClassificationCache.containsKey(cacheKey))
            return externalClassificationCache.get(cacheKey);
    ...
    public String fetchClassificationForCellRemote(int col, int row) {
        ...
        if (externalClassificationCache.containsKey(key))
            return externalClassificationCache.get(key);
        double[] latlon = cellToLatLon(col, row);
        String token = classificationTokenForLatLon(latlon[0], latlon[1]);
        if (token != null)
            externalClassificationCache.put(key, token);
        return token;
    }

    /**
     * Generic API: obtain a single classification token for the specified
     * classification-grid cell. Resolution of where the token comes from (local
     * table, backing grid, remote service, or this layer's stored value) follows
     * this preference and is transparent to callers.
     */
     ...
     * considers a cell traversable when its classification token (as provided by
     * getClassificationTokenForCell) matches one of the allowed tokens
     * (case-insensitive).
     */
