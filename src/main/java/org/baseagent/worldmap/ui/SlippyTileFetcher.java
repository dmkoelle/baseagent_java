...
public class SlippyTileFetcher {
    ...
    // Track tiles currently scheduled/being fetched to avoid duplicate queue
    // entries
    ...
    /**
     * Non-blocking: returns cached Image if available, otherwise schedules a
     * background fetch and returns null.
     */
    ...
    /**
     * Asynchronous fetch that invokes callback when image is available. Priority:
     * lower value means higher priority (0 = highest).
     */
    ...
    private void workerLoop() {
        while (running.get()) {
            ...
            try {
                ...
                Path cachePath = cacheRoot.resolve(String.valueOf(z)).resolve(String.valueOf(xx))
                        .resolve(String.valueOf(yy) + ".png");
                ...
                String urlStr = urlTemplate.replace("{z}", String.valueOf(z)).replace("{x}", String.valueOf(xx))
                        .replace("{y}", String.valueOf(yy));
                ...
                try {
                    if (task != null)
                        pending.remove(keyFor(task.z, task.x, task.y));
                } catch (Exception ex) {
                }
    ...
    private static class FetchTask implements Comparable<FetchTask> {
        ...
        FetchTask(int z, int x, int y, int priority, Consumer<Image> callback, String key) {
            this.z = z;
            this.x = x;
            this.y = y;
