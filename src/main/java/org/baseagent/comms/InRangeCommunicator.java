...
/**
 * Sends the message to all listeners who are within the range specified in the
 * constructor.
 * 
 */
    ...
    public List<MessageListener> getExpectedRecipients(Message message) {
        if (!(message.getOriginalSender() instanceof HasGridPosition))
            throw new IllegalArgumentException(
                    "InRangeCommunicator.sendMessage expects the sender implement HasPosition.");
        ...
        for (MessageListener messageListener : getSimulation().getMessageListeners()) {
            if ((messageListener instanceof HasGridPosition) && !(messageListener.equals(fromAgent))) {
                double distance = BaseAgentMath.distance(((HasGridPosition) messageListener),
                        (HasGridPosition) fromAgent);
                if (distance <= range) {
