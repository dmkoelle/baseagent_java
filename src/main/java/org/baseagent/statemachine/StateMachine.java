...
public class StateMachine implements Behavior {
    ...
    public void addTransition(String originStateName, String destinationStateName, Predicate<Agent> check) {
        Edge<State, Agent> edge = new Edge<>(nodesByStateName.get(originStateName),
                nodesByStateName.get(destinationStateName), check, agent -> {
                });
    ...
    public void addTransition(String originStateName, String destinationStateName, Predicate<Agent> check,
            Consumer<Agent> todo) {
        Edge<State, Agent> edge = new Edge<>(nodesByStateName.get(originStateName),
                nodesByStateName.get(destinationStateName), check, todo);
        network.addEdge(edge);
    }
    ...
    private void transitionStateIfApplicable(Agent agent) {
        Collection<Edge<State, Agent>> applicableEdges = getTransitionsFrom(currentState.getStateName()).stream()
                .filter(edge -> edge.applies(agent)).collect(Collectors.toList());
