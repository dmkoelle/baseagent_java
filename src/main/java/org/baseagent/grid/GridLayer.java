...
public class GridLayer<T> implements Iterable<GridCell<T>> {
    ...
    public enum GridLayerUpdateOption {
        NEXT_BECOMES_CURRENT, NO_SWITCH
    }
    ...
    public Iterator<GridCell<T>> iterator() {
        return new Iterator<GridCell<T>>() {
            ...
            public boolean hasNext() {
                if (x + 1 < parentGrid.getWidthInCells())
                    return true;
                if (y + 1 < parentGrid.getHeightInCells())
                    return true;
                return false;
            }
