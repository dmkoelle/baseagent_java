...
public class GridLayerStep<T> {
    ...
    public double laplacian_3x3(int x, int y, double centerWeight, double adjacentWeight, double diagonalWeight) {
        if (!(get(x, y) instanceof Double)) {
            throw new IllegalArgumentException(
                    "Cannot call Laplacian transform on a grid layer that does not consist of Double values.");
        }
        ...
        return (centerWeight * xy + adjacentWeight * a0 + adjacentWeight * a1 + adjacentWeight * a2
                + adjacentWeight * a3 + diagonalWeight * d0 + diagonalWeight * d1 + diagonalWeight * d2
                + diagonalWeight * d3);
    }
    ...
    public void setEachCell(GridLayerStep<T> conditionalLayer, Function<T, T> f) {
        if ((this.parentGrid.getWidthInCells() != conditionalLayer.parentGrid.getWidthInCells())
                || (this.parentGrid.getHeightInCells() != conditionalLayer.parentGrid.getHeightInCells())) {
            throw new IllegalArgumentException(
                    "The parent grid of this layer and the conditional layer must have the same width and height.");
        }
    ...
    /**
     * Returns a count of the neighbors surrounding the given cell that match the
     * predicate.
     */
    public int count8Neighbors(int x, int y, Predicate<? super T> predicate) {
        int retVal = 0;
        if (predicate.test(get(x - 1, y - 1)))
            retVal++;
        if (predicate.test(get(x, y - 1)))
            retVal++;
        if (predicate.test(get(x + 1, y - 1)))
            retVal++;
        if (predicate.test(get(x - 1, y)))
            retVal++;
        if (predicate.test(get(x + 1, y)))
            retVal++;
        if (predicate.test(get(x - 1, y + 1)))
    ...
    /**
     * Returns a count of the neighbors to the north, south, east, and west of the
     * given cell that match the predicate.
     */
    ...
    public double average9Neighbors(int x, int y) {
        if (!(get(x, y) instanceof Double)) {
            throw new IllegalArgumentException(
                    "Cannot call 'average' method on a grid layer that does not consist of Double values.");
        }
    ...
    /**
     * Returns a count of the number of times the predicate is true across the full
     * layer.
     */
    ...
    /**
     * Test the give predicate. If the predicate is true, return 1, otherwise return
     * 0.
     */
    ...
    /**
     * Randomly places the given thing throughout the GridLayerStep, making sure to
     * not place a thing on a space that already has the same thing.
     */
    ...
    /**
     * Defines a visual arrangement for playing the given thing on the grid. Key: .
     * = empty space, O = Thing
     */
    ...
    public GridPosition getRandomUnoccupiedPosition() {
        ...
        if (numTries == 0) {
            throw new IllegalArgumentException(
                    "Not implemented: Second strategy in getRandomUnoccupiedPosition that starts methodically scanning rows for an empty position, since the random search didn't work for 25% of the cells in the grid.");
        }
