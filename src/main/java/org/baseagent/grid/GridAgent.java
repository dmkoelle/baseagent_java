...
public class GridAgent extends DrawableAgent implements HasFineGridPosition {
    ...
    public GridAgent() {
        ...
        setDrawable(new GridDrawable() {
            ...
            public void draw(GridCanvasContext gcc) {
                ...
                VisualizationLibrary.drawTriangleWithHeading(gcc.getGraphicsContext(), getFineX() * gcc.getXFactor(),
                        getFineY() * gcc.getYFactor(), gcc.getCellWidth(), gcc.getCellHeight(), getHeading(),
                        getColorOrUse(Color.CADETBLUE), getColorOrUse(Color.CADETBLUE).darker());
            }
        });
    }
    ...
    public void moveTo(int x, int y) {
        ...
        // do NOT overwrite fineX/fineY here â€” preserve fractional position for smooth
        // movement
    ...
    /**
     * Place this agent in a random position within the bounding rectangle provided
     * in the parameters
     */
    ...
    public void turnLeft() {
        this.rotateDelta(-BaseAgentMath.HALF_PI);
    }
    ...
    public void turnAround() {
        this.rotateDelta(BaseAgentMath.PI);
    }

    public void moveLeft() {
        this.cellX = getGrid().getBoundsPolicy().boundX(this.cellX - 1);
        setHeading(BaseAgentMath.PI);
    }
    ...
    public void moveUp() {
        this.cellY = getGrid().getBoundsPolicy().boundY(this.cellY - 1);
        setHeading(BaseAgentMath.HALF_PI);
    }

    public void moveDown() {
        this.cellY = getGrid().getBoundsPolicy().boundY(this.cellY + 1);
        setHeading(BaseAgentMath.THREE_HALF_PI);
    }
    ...
    public void moveForward(double distance) {
        moveTo(this.cellX + (int) (distance * Math.cos(getHeading())),
                this.cellY + (int) (distance * Math.sin(getHeading())));
    }
    ...
    public boolean isOn(Object value) {
        return isOn(Grid.DEFAULT_GRID_LAYER, value);
    }

    public boolean isOn(String gridLayerName, Object value) {
        return getGrid().getGridLayer(gridLayerName).get(getCellX(), getCellY()).equals(value);
    }

    public void setCell(Object value) {
        setCell(Grid.DEFAULT_GRID_LAYER, value);
    }

    public void setCell(String gridLayerName, Object value) {
        getGrid().getGridLayer(gridLayerName).set(getCellX(), getCellY(), value);
    }
