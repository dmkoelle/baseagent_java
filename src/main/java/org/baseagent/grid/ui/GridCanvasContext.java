...
public class GridCanvasContext {
    ...
    public GridCanvasContext(Simulation simulation, Grid grid, GridCanvas gridCanvas, int cellWidth, int cellHeight,
            int cellXSpacing, int cellYSpacing) {
    ...
    /**
     * Compute the inclusive bounds of visible cells for the current view
     * parameters. Returns int[]{minCol, minRow, maxCol, maxRow} clamped to the grid
     * extents.
     */
    ...
    /**
     * Convert a cell upper-left to world pixel coordinates (untranslated, scaled by
     * zoom). Returns {worldX, worldY} where worldX/worldY are in world pixels.
     */
    ...
    /**
     * Convert the center of a cell to world pixel coordinates. Returns {worldX,
     * worldY} for the center point in world pixels.
     */
    ...
    /**
     * Convert world pixel coordinates to screen pixel coordinates using the current
     * view offset. Screen coords are what should be passed to GraphicsContext
     * drawing calls.
     */
    ...
    /**
     * Convert a cell to screen pixel coordinates for its center. Returns {screenX,
     * screenY}.
     */
    ...
    /**
     * Convert screen coords to the cell indices (col,row) that contain that screen
     * point.
     */
    public int[] screenToCell(double screenX, double screenY) {
        ...
        if (col < 0)
            col = 0;
        if (col >= cols)
            col = Math.max(0, cols - 1);
        if (row < 0)
            row = 0;
        if (row >= rows)
            row = Math.max(0, rows - 1);
        return new int[] { col, row };
    }

    /**
     * Create an EmbeddedContext for an embedded grid that is drawn inside the given
     * parent cell (parentCol,parentRow). The embedded grid has embeddedCols x
     * embeddedRows cells. extraScale is a multiplier: 1.0 means the embedded grid
     * exactly fills the parent cell; >1.0 zooms in; <1.0 makes the embedded grid
     * smaller than the parent cell and centers it.
     */
    public EmbeddedContext createEmbeddedContext(int parentCol, int parentRow, int embeddedCols, int embeddedRows,
            double extraScale) {
        return new EmbeddedContext(this, parentCol, parentRow, embeddedCols, embeddedRows, extraScale);
    }

    /**
     * Helper that represents an embedded-grid coordinate space inside a parent
     * cell.
     */
    public static class EmbeddedContext {
        ...
        EmbeddedContext(GridCanvasContext parent, int parentCol, int parentRow, int embeddedCols, int embeddedRows,
                double extraScale) {
            this.parent = parent;
            this.parentCol = parentCol;
            this.parentRow = parentRow;
            this.embeddedCols = Math.max(1, embeddedCols);
            this.embeddedRows = Math.max(1, embeddedRows);
            this.extraScale = (extraScale <= 0.0) ? 1.0 : extraScale;

            double[] ul = parent.cellToWorld(parentCol, parentRow);
            this.parentULWorldX = ul[0];
            this.parentULWorldY = ul[1];
        ...
        public double getEmbeddedCellScreenWidth() {
            // embeddedCellWorldW is in world pixels; convert to screen by subtracting
            // offsets already used via worldToScreen
            return this.embeddedCellWorldW;
        }

        public double getEmbeddedCellScreenHeight() {
            return this.embeddedCellWorldH;
        }

        /**
         * Which embedded cell contains the given screen point? Returns {subCol,
         * subRow}.
         */
        public int[] screenToEmbeddedCell(double screenX, double screenY) {
            ...
            if (subCol < 0)
                subCol = 0;
            if (subCol >= embeddedCols)
                subCol = embeddedCols - 1;
            if (subRow < 0)
                subRow = 0;
            if (subRow >= embeddedRows)
                subRow = embeddedRows - 1;
