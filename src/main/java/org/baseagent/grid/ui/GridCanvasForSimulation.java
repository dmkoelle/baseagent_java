...
public class GridCanvasForSimulation extends GridCanvas implements SimulationListener {
    ...
    public GridCanvasForSimulation(Simulation simulation, Grid grid, int cellWidth, int cellHeight, int cellXSpacing,
            int cellYSpacing) {
    ...
    protected void update0(GridCanvasContext gcc) {
        ...
        // Then draw beacons
        if (drawBeacons) {
            List<GridDrawable> drawableBeacons = simulation.getBeacons().stream().filter(a -> a instanceof GridDrawable)
                    .map(a -> (GridDrawable) a).collect(Collectors.toList());
        ...
        // Then draw agents
        if (drawAgents) {
            List<GridDrawable> drawableAgents = simulation.getAgents().stream().filter(a -> a instanceof GridDrawable)
                    .map(a -> (GridDrawable) a).collect(Collectors.toList());
        ...
        if (drawToasts) {
            ...
            List<GridDrawable> drawableToasts = getToasts().stream().filter(a -> a instanceof GridDrawable)
                    .map(a -> (GridDrawable) a).collect(Collectors.toList());
            drawableToasts.stream().filter(toast -> ((Toast) toast).isActive(getSimulation()))
                    .forEach(toast -> toast.draw(gcc));
