...
public class GridNetworkRenderer<T, R> implements GridLayerRenderer {
    ...
    private BiConsumer<GridCanvasContext, Node<T>> nodeRenderer = (gcc, node) -> {
        ...
        if (posop.isPresent()) {
            ...
            g.fillOval(posop.get().getCellX() * gcc.getCellWidth(), posop.get().getCellY() * gcc.getCellHeight(),
                    gcc.getCellWidth(), gcc.getCellHeight());
        }
    };
    ...
    private BiConsumer<GridCanvasContext, Edge<T, R>> edgeRenderer = (gcc, edge) -> {
        ...
        Optional<HasGridPosition> destinationPosop = GridNetworkMath
                .getPossibleGridPositionForNode(edge.getDestinationNode());

        if (sourcePosop.isPresent() && destinationPosop.isPresent()) {
            GraphicsContext g = gcc.getGraphicsContext();
            g.setStroke(Color.LIGHTBLUE);
            g.strokeLine(sourcePosop.get().getCellX() * gcc.getCellWidth(),
                    sourcePosop.get().getCellY() * gcc.getCellHeight(),
                    destinationPosop.get().getCellX() * gcc.getCellWidth(),
                    destinationPosop.get().getCellY() * gcc.getCellHeight());
        }
    };
