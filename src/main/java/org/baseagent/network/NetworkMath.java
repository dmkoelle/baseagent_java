...
public class NetworkMath<T, R> {
     ...
     * @return List of Node<T> representing the shortest path, or empty list if no
     *         path exists
     */
    public static <T, R> List<Node<T>> findShortestPath(Network<T, R> network, Node<T> startNode, Node<T> endNode) {
        ...
        PriorityQueue<NodeDistance<T>> pq = new PriorityQueue<>(Comparator.comparingDouble(nd -> nd.distance));
    ...
    /**
     * Gets the weight of an edge between two nodes TODO: This method should be
     * specific for GridNetworkMath
     */
